...
const gradientMapping: Record<string, string> = {
  blue: "linear-gradient( hsl(198, 1000%, 79%),hsl(225, 8%, 14%))",
...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className }) => {
  ...
  return (
    <div
      className={`grid gap-[5em] grid-cols-3 md:grid-cols-4 lg:grid-cols-6 mx-auto py-[3em] overflow-visible ${
        className || ""
      }`}
    >
      {items.map((item, index) => (
        <button
          ...
          className={`relative bg-transparent outline-none w-[3.5em] h-[3.5em] sm:w-[5em] sm:h-[5em] md:w-[6.5em] md:h-[6.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
            item.customClass || ""
          }`}
        >
          <span
          ...
          >
            <span
              className="m-auto w-[1.5em] h-[1.5em]  md:w-[2.5em] md:h-[2.5em] flex items-center justify-center"
              aria-hidden="true"
            >
              {item.icon}
            </span>
